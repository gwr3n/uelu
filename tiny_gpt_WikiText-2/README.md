# WikiText-2 Token-Level GPT Controlled Study

This folder contains a conventional language-modeling extension of the paper's Tiny Shakespeare experiment. It compares `GELU` with `ReLU`, `SiLU`/Swish, fixed-width `UELU`, trainable-width `TUELU`, and centered dynamic GELU (`DGELU`) in the feedforward blocks of a compact decoder-only Transformer trained on WikiText-2.
...
```bash
python gelu_vs_uelu_wikitext2_gpt.py --train-text-file train.txt --val-text-file validation.txt
```

Choose a tokenizer vocabulary:

```bash
python gelu_vs_uelu_wikitext2_gpt.py --encoding bpe8k
python gelu_vs_uelu_wikitext2_gpt.py --encoding bpe16k
python gelu_vs_uelu_wikitext2_gpt.py --encoding gpt2
```
