"""
Tiny character-level GPT experiment for the GELU/UELU computational study.
...
class CharDataset:
    def __init__(self, text: str, train_fraction: float, device: torch.device) -> None:
        ...
        self.train_data = encoded[:split].to(device)
        self.val_data = encoded[split:].to(device)
...
def load_text(args: argparse.Namespace) -> str:
    ...
    if not target.exists():
        print(f"Downloading Tiny Shakespeare to {target}")
        urllib.request.urlretrieve(TINY_SHAKESPEARE_URL, target)
    return target.read_text(encoding="utf-8")


@torch.no_grad()
def estimate_loss(model: TinyGPT, dataset: CharDataset, args: argparse.Namespace) -> Tuple[float, float]:
    model.eval()
...
@torch.no_grad()
def measure_activation_regions(model: nn.Module, dataset: CharDataset, args: argparse.Namespace):
...
def run_one_setting(args: argparse.Namespace, activation: str, seed: int, dataset: CharDataset, device: torch.device):
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Controlled GELU/UELU tiny character-level GPT study.")
...
def main() -> None:
    ...
    text = load_text(args)
    dataset = CharDataset(text, args.train_fraction, device)
    print(f"Loaded {len(text):,} characters with vocabulary size {dataset.vocab_size}")
