"""
Compact ViT CIFAR-100 experiment for the GELU/UELU computational study.

Examples
--------
Smoke test:
    python gelu_vs_uelu_vit.py --dry-run
...
class TransformerBlock(nn.Module):
    def __init__(
        ...
        self.ffn = FeedForward(dim, mlp_dim, activation, uelu_beta, trainable_raw_beta, trainable_beta_min, dropout)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        attended, _ = self.attn(self.attn_norm(x), self.attn_norm(x), self.attn_norm(x), need_weights=False)
        x = x + attended
...
def make_loaders(args: argparse.Namespace) -> Tuple[DataLoader, DataLoader, DataLoader]:
    ...
    if args.dry_run:
        ...
        test_set = Subset(test_set, list(range(min(args.dry_run_val_size, len(test_set)))))
    ...
    return (
        DataLoader(train_set, shuffle=True, drop_last=False, **loader_kwargs),
        DataLoader(val_set, shuffle=False, drop_last=False, **loader_kwargs),
        DataLoader(test_set, shuffle=False, drop_last=False, **loader_kwargs),
    )
...
@torch.no_grad()
def evaluate(model, loader, criterion, device, label: str) -> Tuple[float, float]:
    model.eval()
    loss_sum = 0.0
    correct = 0
    total = 0
...
@torch.no_grad()
def measure_activation_regions(model, loader, device, max_batches: int):
...
def run_one_setting(args, activation, seed, train_loader, val_loader, test_loader, device):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        val_loss, val_accuracy = evaluate(model, val_loader, criterion, device, f"epoch {epoch} val")
        scheduler.step()
    ...
    closed, transition, open_fraction = measure_activation_regions(model, val_loader, device, args.region_batches)
    trainable_beta = get_trainable_beta(model)
    final = FinalMetrics(
        activation=activation,
        seed=seed,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Controlled GELU/UELU compact ViT CIFAR-100 study.")
...
def main() -> None:
    ...
    for seed in args.seeds:
        for activation in args.activations:
            epoch_rows, final = run_one_setting(
                args, activation, seed, train_loader, val_loader, test_loader, device
            )
            all_epoch_rows.extend(epoch_rows)
            final_rows.append(final)
            print(
                f"final {activation} seed={seed}: test={100 * final.test_accuracy:.2f}% "
                f"best_val={100 * final.best_val_accuracy:.2f}% best_epoch={final.best_epoch}"
            )
